...
exports.updateVideo = asyncHandler(async (req, res, next) => {
    const {id} = req.params;
    const video = await Video.findByIdAndUpdate(id, {$set: req.body}, {
        new: true,
        runValidators: true
    })
    if (!video) return next(new ErrorResponse(`No video with that id of ${id}`))
...
exports.deleteVideo = asyncHandler(async (req, res, next) => {
    ...
    if (!video)
        return next(new ErrorResponse(`No video with id of ${id}`, 404))
    await video.remove()
...
//
//     const {err, message} = await isUploadLimit(req);
//     if (err) return next(new ErrorResponse(message))
//
//     let video = req.body;
//     if (!video.overlay) {
...
//     })
//
//     if (!video) return next(new ErrorResponse(`No video with that id of ${req.params.id}`))
//
//     res.status(200).json({success: true, data: video})
// })
//
...
//     const video = await Video.findById(req.params.id)
//     if (!video) return next(new ErrorResponse(`No video with id of ${req.params.id}`, 404))
//
//     if (!req.files) {
...
//         return next(new ErrorResponse(`Please upload an image less than ${process.env.MAX_FILE_UPLOAD / 1000 / 1000}mb`, 404))
//     }
//
...
//     //       )
//     //     }
//     fs.unlink(`${process.env.FILE_UPLOAD_PATH}/thumbnails/${video.thumbnailUrl}`, async (err) => {
//         // if (err) {
//         //   return next(
//         //     new ErrorResponse(
//         //       `Something went wrong, couldn't delete video photo`,
